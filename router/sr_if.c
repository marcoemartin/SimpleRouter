 ...
 * Scope: Global
 *
 * Given an IP, find interface entry. Returns NULL if no entry is found.
 *
 *---------------------------------------------------------------------*/
struct sr_if* sr_get_interface_by_ip(struct sr_instance* sr, const uint32_t ip_needle) {
...
/*---------------------------------------------------------------------
 * Method: sr_get_interface_by_eth_addr
 * Scope: Global
 *
 * Given an eth address, find interface entry. Returns NULL if no entry is found.
 *
 *---------------------------------------------------------------------*/
struct sr_if* sr_get_interface_by_eth_addr(struct sr_instance* sr, const unsigned char* eth_address) {
    ...
    while(if_walker != NULL) {
        if(is_eth_address_equal(eth_address, if_walker->addr)) {
            return if_walker;
        }
