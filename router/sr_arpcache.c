...
void handle_arpreq(struct sr_instance* sr, struct sr_arpreq* arp_request) {
    ...
    while(current_interface != NULL) {
        ...
        set_eth_address_with(eth_header->ether_shost, current_interface->addr);

        /* TODO: code review; right sender ip???? */
        arp_header->ar_sip = current_interface->ip; /* set sender ip */
        set_eth_address_with(arp_header->ar_sha, current_interface->addr);

        /* TODO: debug remove */
        printf("handle_arpreq/Sending ARP request packet to %s\n", current_interface->name);

        sr_send_packet(sr, packet, packet_size, current_interface->name);
